#generate Hill function dose response curves
import scipy
import numpy as np
...
parmesan = {'legend.fontsize': 'x-large',
          'figure.figsize': (7, 5),
         'axes.labelsize': 'xx-large',
         'axes.titlesize':'xx-large',
         'xtick.labelsize':'xx-large',
         'ytick.labelsize':'xx-large'}
plt.rcParams.update(parmesan)

en = input("Which regression is preferred?  Normal (N) or Cauchy (C):  ")
steepCorrection = input("Steepness correction?  Yes (1) or No (0):  ")
if steepCorrection == '1':
    corrStrength = float(input("Correction strength between 0 and 1:  "))
else:
    corrStrength = 0
sdr = input("Same drug correction?  Yes (1) or No (0):  ")
...
def expVariance(vals):
    ...
    var = sum(diff)/(len(diff)-1) + 0.0001
    #print(vals, var)
    return var
...
def hill_4(x, t, y, w):
    ...
    if en == 'N' or en == 'n':
        reg = sum(w[i]*(x[3]+x[0]*t[i]**x[1]/(x[2]**x[1]+t[i]**x[1]) - y[i])**2 for i in range(len(t)))
    else:
        reg = sum(w[i]*np.log(1+(x[3]+x[0]*t[i]**x[1]/(x[2]**x[1]+t[i]**x[1]) - y[i])**2) for i in range(len(t)))
...
def hill_3(x, t, y, w):
    ...
    if en == 'N' or en == 'n':
        reg = sum(w[i] * (x[0] * t[i] ** x[1] / (x[2] ** x[1] + t[i] ** x[1]) - y[i]) ** 2 for i in range(len(t)))

    else:
        reg = sum(w[i] * np.log(1+(x[0] * t[i] ** x[1] / (x[2] ** x[1] + t[i] ** x[1]) - y[i]) ** 2) for i in range(len(t)))

    return reg

def refitn(x, t, y, w, ec):
    reg = sum(w[i] * np.log(1+(x[0] * t[i] - x[0] * ec  - y[i]) ** 2) for i in range(len(t)))
    return reg
...
def plotIndFits_4(params, conc, resp, logging, j):
    ...
    plt.xlabel('Drug Concentration (mM)')
    plt.ylabel('Fractional Response')
    #plt.title('Empirical Fit: Drug ' +str(j+1))

    plt.show()

def plotIndFits_3(params, conc, resp, logging, j, csv):
    index = np.linspace(min(conc), max(conc), 50000)
    ...
    #plt.title('Empirical Fit: Drug ' +str(j+1))
    sns.despine()
    #filepic = "C:/Users/Richard/Desktop/DrugCombosKalin/updatedFiles_Nov2020/" + csv + str(j) +"_.png"
    filepic = "C:/Users/Richard/Desktop/GlucoseLimitation/" + "larvae_Smarcescens" + str(j) +"_.png"
    projPlot.savefig(filepic, bbox_inches="tight")
...
def plotComparisons(x, theoretical, theoError, observed, obsError,logging, csv):
    ...
    #plt.title(csv)
    sns.despine()
    #filepic = "C:/Users/Richard/Desktop/DrugCombosKalin/updatedFiles_Nov2020/" + csv + ".png"
    filepic = "C:/Users/Richard/Desktop/GluocoseLimitation/" + "larvae_Smarcescens" + "_.png"
    projPlot.savefig(filepic, bbox_inches="tight")
    plt.show()
...
def plotComboError(comboError, minC, maxC, logging, csv):
    ...
    plt.plot(index[1:], results)
    if logging == 1:
        plt.xscale('log')
    plt.xlabel('Drug Concentration (mM)')
    plt.ylabel('Standard Error')
    sns.despine()
    #filepic = "C:/Users/Richard/Desktop/DrugCombosKalin/updatedFiles_Nov2020/" + csv + "e.png"
    filepic = "C:/Users/Richard/Desktop/GlucoseLimitation" + "larvae_Smarcescens" + "e.png"
    projPlot.savefig(filepic, bbox_inches="tight")
...
def plotComboAll(concen, resp, combinedEffect, upperConfidence, lowerConfidence, logging, csv):
    ...
    projPlot = plt.figure()
    plt.plot(index, eval(combinedEffect), color='k', label='Expected')
    plt.plot(index[1:], upper, color='k', linestyle='--')
    plt.plot(index[1:], lower, color='k', linestyle='--')
    plt.scatter(concen, resp, color='blue', label='Empirical')
    plt.legend()
    if logging == 1:
        plt.xscale('log')
    ...
    sns.despine()
    #filepic = "C:/Users/Richard/Desktop/DrugCombosKalin/updatedFiles_Nov2020/" + csv + "2.png"
    filepic = "C:/Users/Richard/Desktop/GlucoseLimitation" + "larvae_Smarcescens" + "2.png"
    projPlot.savefig(filepic, bbox_inches="tight")
...
def sameDrugCorrection(xVals, theoretical, para, frac):
    corrected = []
    xList = []
    for i in range(len(xVals)):
        xList.append(xVals[i][0])

    for i in range(len(xList)):
        x = xList[i]
        fract = frac[i][0]
        y = x ** para[1] / (para[2] ** para[1] + x ** para[1]) - (fract * x) ** para[1] / (para[2] ** para[1] + (fract * x) ** para[1]) - ((1-fract) * x) ** para[1] / (para[2] ** para[1] + ((1-fract) * x) ** para[1]) + \
        (fract * x) ** para[1] / (para[2] ** para[1] + (fract * x) ** para[1]) * ((1-fract) * x) ** para[1] / (para[2] ** para[1] + ((1-fract) * x) ** para[1])
        corrected.append(y + theoretical[i])
    return corrected
...
def main():
    ...
    csvname = "MDAMB231_CisptOlapB02"
    #file = "C:/Users/Richard/Desktop/DrugCombosKalin/updatedFiles_Nov2020/" + csvname + ".csv"  # import data file
    file = "C:/Users/Richard/Desktop/GlucoseLimitation/larvae_Smarcescens.csv"
    ...
    for j in range(len(concentrations)-1):  # iterate through the number of data sets (number of data columns / 3)
        ...
        res = minimize(hill_4, x0, args=(concentration, response, weight), method='SLSQP',
                       bounds=((0, 1), (0, None), (0, None), (None, None)), tol=None,
        ...
        if interceptCorrection == 0:
            ...
            res = minimize(hill_3, params[0:3], args=(concentration, response, weight), method='BFGS', tol=None,
                           options={'maxiter': 10000, 'ftol': tol})
            params = res.x
            ...
            params_reduced = [params[0], params[1], params[2]]
        print('these are the parameters: ', params)
        if steepCorrection == '0':
            ...
            for l in range(len(response)):
                if params[2]/50 <= concentration[l] <= params[2]*50:
                    concentration2.append(concentration[l])
                    response2.append(np.log(response[l] / (params[0] - response[l])))
                    weight2.append(weight[l])
            for m in range(len(response2)):
                if isnan(response2[m]) == False:
                    concentration3.append(np.log(concentration2[m]))
                    response3.append(response2[m])
                    weight3.append(weight2[m])

            correc = minimize(refitn, np.array([params[1]]), args=(concentration3, response3, weight3, np.log(params[2])), method='BFGS', tol=None,
                           options={'maxiter': 10000, 'ftol': tol})
            parms = correc.x
            plt.scatter(concentration3, response3)
            xvals = np.linspace(min(concentration3), max(concentration3), 100)
            plt.plot(xvals, parms[0] * xvals - parms[0] * np.log(params[2]))

            plt.show()


            params[1] = corrStrength*parms[0] + (1-corrStrength)*params[1]

            rss = hill_3(params, concentration, response, weight) / (len(concentration) - len(params))
            print('var is', np.sqrt(np.diag(correc.hess_inv))) #* np.sqrt(rss))
            params_reduced = [params[0], params[1], params[2]]
        print('these are the parameters 2: ', params)
        # print(res.hess_inv)



        covariance = np.sqrt(np.diag(res.hess_inv)) #* np.sqrt(rss)
        print('other var is', covariance)
        ...
        if interceptCorrection == 0:
            ...
            plotIndFits_3(params, concentration, response, logging, j, csvname)  # plot curves with data
    ...
    if fixedRatio == True:
        ...
        plotComboAll(concentrationCombo, responseCombo, combinedEffect, upperConfidence, lowerConfidence, logging, csvname)
    ...
    for u in range(len(groupedWeightCombo)):
        ...
        # print(allError)
        plotComboError(allError, minConcentration, maxConcentration, logging, csvname)
    ...
    if sdr == '1':
        print(groupedConcentrationCombo, theoreticalResponseCombo, params_reduced, groupedDrugRatio)
        theoreticalResponseCombo = sameDrugCorrection(groupedConcentrationCombo, theoreticalResponseCombo, params_reduced, groupedDrugRatio)
